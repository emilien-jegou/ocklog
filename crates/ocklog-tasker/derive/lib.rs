...
pub fn derive_tasker_provide(input: TokenStream) -> TokenStream {
    ...
    let expanded = fields.iter().map(|f| {
        ...
        quote! {
            impl ::ocklog_tasker::worker::ExtractsFrom<#struct_name> for #field_ty {
...
pub fn derive_tasker_context(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl<__C> ::ocklog_tasker::worker::ExtractsFrom<__C> for #name
        where
            #( #field_types: ::ocklog_tasker::worker::ExtractsFrom<__C> ),*
        {
            fn extract(ctx: &__C) -> Self {
                Self {
                    #(
                        #field_names: <#field_types_2 as ::ocklog_tasker::worker::ExtractsFrom<__C>>::extract(ctx)
                    ),*
                }
            }
        }
...
pub fn tasker_registry(input: TokenStream) -> TokenStream {
    ...
    let spawn_bounds = mapped_events.iter().flat_map(|(_, ty, listeners)| {
        listeners.iter().map(move |listener_ty| {
            quote! {
                <#listener_ty as ::ocklog_tasker::worker::Listener<#ty, EventSender>>::Context: ::ocklog_tasker::worker::ExtractsFrom<C>
            }
        })
    });
    ...
    let match_arms = mapped_events.iter().map(|(name, ty, listeners)| {
        let listener_spawns = listeners.iter().map(|listener_ty| {
            quote! {
                let ctx_extracted = <<#listener_ty as ::ocklog_tasker::worker::Listener<#ty, EventSender>>::Context as ::ocklog_tasker::worker::ExtractsFrom<C>>::extract(&*c);
                let ev_clone = ev.clone();
                let tx = tx_clone.clone();

                ::tokio::spawn(
                    async move {
                        let span = ::ocklog_tasker::reexport::tracing::info_span!(
                            "listener_handle",
                            event_type = stringify!(#name),
                            listener = stringify!(#listener_ty)
                        );

                        use ::ocklog_tasker::reexport::tracing::Instrument;
                        async {
                            let res = <#listener_ty as ::ocklog_tasker::worker::Listener<#ty, EventSender>>::handle(ev_clone, ctx_extracted, tx).await;
                            match res {
                                Ok(_) => {
                                    ::ocklog_tasker::reexport::tracing::trace!("Listener completed successfully");
                                }
                                Err(e) => {
                                    ::ocklog_tasker::reexport::tracing::error!(error = ?e, "Listener failed");
                                }
                            }
                        }
                        .instrument(span)
    ...
    let expanded = quote! {
        ...
        pub struct EventSender {
            tx: ::ocklog_tasker::reexport::async_channel::Sender<Event>,
        }
        ...
        impl EventSender {
                ...
                Event: From<E>,
            {
                ...
                ::ocklog_tasker::reexport::tracing::debug!(?ev, "EventSender sending event");
                self.tx.try_send(ev).map_err(|e| match e {
                    ::ocklog_tasker::reexport::async_channel::TrySendError::Full(_) => unreachable!("unbounded channel cannot be full"),
                    ::ocklog_tasker::reexport::async_channel::TrySendError::Closed(val) => ::tokio::sync::mpsc::error::SendError(val),
                })
            }

            pub fn shutdown(&self) -> Result<(), ::tokio::sync::mpsc::error::SendError<Event>> {
                ::ocklog_tasker::reexport::tracing::info!("EventSender sending Shutdown signal");
        ...
        pub struct EventReceiver {
            rx: ::ocklog_tasker::reexport::async_channel::Receiver<Event>,
        }
        ...
        impl EventReceiver {
            ...
            pub fn try_recv(&self) -> Result<Event, ::tokio::sync::mpsc::error::TryRecvError> {
                self.rx.try_recv().map_err(|e| match e {
                    ::ocklog_tasker::reexport::async_channel::TryRecvError::Empty => ::tokio::sync::mpsc::error::TryRecvError::Empty,
                    ::ocklog_tasker::reexport::async_channel::TryRecvError::Closed => ::tokio::sync::mpsc::error::TryRecvError::Disconnected,
                })
            }
        }
        ...
        impl EventRegistry {
            ...
            {
                let ev = Event::from(event);
                ::ocklog_tasker::reexport::tracing::debug!(?ev, "EventRegistry sending event");
                self.tx.try_send(ev).map_err(|e| match e {
                    ::ocklog_tasker::reexport::async_channel::TrySendError::Full(_) => unreachable!("unbounded channel cannot be full"),
                    ::ocklog_tasker::reexport::async_channel::TrySendError::Closed(val) => ::tokio::sync::mpsc::error::SendError(val),
                })
                ...
                #( #spawn_bounds, )*
            {
                ...
                let (req_tx, req_rx) = ::ocklog_tasker::reexport::async_channel::unbounded::<Event>();
                let (ev_tx, ev_rx) = ::ocklog_tasker::reexport::async_channel::unbounded::<Event>();
                ...
                let handle = ::tokio::spawn(async move {
                    ...
                    async move {
                        while let Ok(event) = req_rx.recv().await {
                            ::ocklog_tasker::reexport::tracing::debug!(?event, "Processing event");
                            match event {
                                Event::Shutdown => {
                    ...
                    .instrument(::ocklog_tasker::reexport::tracing::info_span!("event_registry_worker_loop"))
                    .await;
                });
