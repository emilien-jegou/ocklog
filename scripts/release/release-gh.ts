...
const WORKSPACE_CRATES = ["ocklog", "syndiff"];
...
async function getVersion(): Promise<string> {
  const cargoTomlPath = join(ROOT, "crates/ocklog/Cargo.toml");
  ...
  if (!version) {
    die("Unable to identify current version in crates/ocklog/Cargo.toml.");
  }
  return version;
}
